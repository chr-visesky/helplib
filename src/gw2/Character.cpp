...
uint16_t Character::GetID() const {
...
bool Character::IsWeaponDrawn() const {
    return m_ptr && m_ptr->isWeapDrawn;
}
...
float Character::GetCurrentBarrier() const {
    return m_ptr ? m_ptr->currBarrier : 0;
}
...
EquipItem Character::GetEquipItems() const {
    EquipItem item;
    if (m_ptr) {
        item.dataList = &m_ptr->equipItemDataList;
        item.iterator = item.dataList->begin();
    }
    return item;
}

EquipItem Character::GetEquipItem(GW2::EquipmentSlot slot) {
    EquipItem item;
    if (m_ptr && slot < GW2::EQUIP_SLOT_END && m_ptr->equipItemDataList[slot]) {
        item.m_ptr = m_ptr->equipItemDataList[slot].get();
    }
    return item;
}

GW2::CharacterRank Character::GetRank() {
    using namespace GameData;
    using namespace GW2;

    if (!m_ptr) return CHAR_RANK_NORMAL;

    uint32_t flags = m_ptr->asFlags;

    if (flags & AS_FLAG_LEGENDARY) return CHAR_RANK_LEGENDARY;
    else if (flags & AS_FLAG_CHAMPION) return CHAR_RANK_CHAMPION;
    else if (flags & AS_FLAG_ELITE) return CHAR_RANK_ELITE;
    else if (flags & AS_FLAG_VETERAN) return CHAR_RANK_VETERAN;
    else if (flags & AS_FLAG_AMBIENT) return CHAR_RANK_AMBIENT;

    return CHAR_RANK_NORMAL;
}

void Character::TriggerSkill(GW2::SkillSlot slot) {
    TriggerSkillDown(slot);
    TriggerSkillUp(slot);
}

void Character::TriggerSkillDown(GW2::SkillSlot slot) {
    if (!m_ptr || !m_ptr->pSkillbar) return;

    const GW2LIB::Mems *offsets = GetMain()->GetGameOffsets();
    m_ptr->pSkillbar.call<void>(offsets->skillbarVtSkillDown, (uint32_t)slot, (uint32_t)0, (uint32_t)0);
}

void Character::TriggerSkillUp(GW2::SkillSlot slot) {
    if (!m_ptr || !m_ptr->pSkillbar) return;

    const GW2LIB::Mems *offsets = GetMain()->GetGameOffsets();
    m_ptr->pSkillbar.call<void>(offsets->skillbarVtSkillUp, (uint32_t)slot);
}
