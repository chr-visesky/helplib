...
namespace
{
GW2LIB::Vector3 Subtract(const GW2LIB::Vector3& lhs, const GW2LIB::Vector3& rhs)
{
    return GW2LIB::Vector3(lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z);
}

GW2LIB::Vector3 Normalize(const GW2LIB::Vector3& v)
{
    const float len = std::sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
    if (len <= 0.0f) {
        return GW2LIB::Vector3();
    }
    const float inv = 1.0f / len;
    return GW2LIB::Vector3(v.x * inv, v.y * inv, v.z * inv);
}
}
...
#ifdef _DEBUG
    hl::LogError(hl::Color::LightGray, file, func, line, fmt_dbg, msg, code, er->ExceptionAddress);
#else
    hl::LogError(hl::Color::LightGray, fmt_rel, msg);
#endif
...
void Gw2HackMain::GameHook()
{
    ...
    m_gameData.mouseInWorld = asctx.get<GW2LIB::Vector3>(m_pubmems.asctxStoW);
