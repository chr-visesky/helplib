...
GameData::ObjectData::ObjectData() {
    auto main = GetMain();
    if (main) main->m_decodedStrings.insert(&name);
}

GameData::ObjectData::~ObjectData() {
    auto main = GetMain();
    if(main) main->m_decodedStrings.erase(&name);
}
...
GameData::AgentData *GameData::GetAgentData(hl::ForeignClass pAgent)
{
    if (pAgent) {
        ...
        uint32_t agentId = pAgent.get<uint32_t>(offsets->agentId);
